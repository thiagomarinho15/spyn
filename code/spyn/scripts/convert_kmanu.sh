...
$1 ~ /loop_/ { loop_switch=1; ivar=0; jvar=0; next }
...
END {
 ...
 print "                      outdir = '\''/tmpdir'\''"
 print "                  pseudo_dir = '\''/PPdir'\''"
 print "                      prefix = '\''"FILE_PREFIX"'\''"
 print "                   verbosity = '\''default'\''"
 print "                     tstress = .true."
 print "                     tprnfor = .true."
 print " /"
 print " &SYSTEM"
...
if (SG==2) {
 ...
 print "                 electron_maxstep = pystepscf"
 print "                 mixing_mode = '\''plain'\''"
 print "                 mixing_beta = 0.5"
 print "             diagonalization = '\''pydiag'\''"
 print " /"
 print "\nATOMIC_SPECIES"
 for (i=0; i<ntyp; i++) printf "  %3s  %14.10f  %s.pbe-tm-gipaw.UPF\n", AtomTyp[i], Atoms[AtomTyp[i]], AtomTyp[i];
