...
class qe(Dirs, get_dir):
    ...
    def pw_output(self):
        ...
        try:
            ...
            pw_stdin  = open('{}pwin.in'.format(self.tmp_dir),  'r')
            pw_stdout = open('{}pwout.out'.format(self.tmp_dir), 'w')
            proc_pw = subprocess.Popen(
                ['mpirun', '-np', str(nproc), 'pw'],
                stdin=pw_stdin, stdout=pw_stdout, stderr=pw_stdout)
    ...
    def gipaw(self):
        ...
        if debug == 'pass': #se o CIF foi aberto, rode normal
            ...
            try:
                ...
                gipaw_stdin  = open('{}gipawin.in'.format(self.tmp_dir),  'r')
                gipaw_stdout = open('{}gipawout.out'.format(self.tmp_dir), 'w')
                proc_gipaw = subprocess.Popen(
                    ['mpirun', '-np', str(nproc), 'gipaw'],
                    stdin=gipaw_stdin, stdout=gipaw_stdout, stderr=gipaw_stdout)
                cnt = 0
                self.defreeze()
                ...
                gipaw2_stdin  = open('{}gipawin.in'.format(self.tmp_dir),  'r')
                gipaw2_stdout = open('{}gipawout.out'.format(self.tmp_dir), 'w')
                proc_gipaw2 = subprocess.Popen(
                    ['mpirun', '-np', str(nproc), 'gipaw'],
                    stdin=gipaw2_stdin, stdout=gipaw2_stdout, stderr=gipaw2_stdout)
                cnt = 0
                self.defreeze()
